...
def processCommand (c):
    ...
    #     response = requests.get("https://newsapi.org/v2/top-headlines?country=us&apiKey=API_KEY")
    #     if response.status_code == 200:
    #         data = response.json()  # Convert JSON response to a Python dict
    #         articles = data.get("articles", [])
    
    #         if articles:
    #             print("Top Headlines:\n")
    #             for i, article in enumerate(articles[:10], start=1):  # first 10 headlines
    #                 speak(article['title'])
    #         else:
